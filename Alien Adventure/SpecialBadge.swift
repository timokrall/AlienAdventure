...
class SpecialBadge: Badge {
    ...
    enum BadgeAnimation: Int {
        case GrowandSrink = 0, Rotate = 1, Shake = 2
    }
    
    override init(requestType: UDRequestType) {
        
        // Change texture from magenta to teal
        let textureTeal = SKTexture(imageNamed: "BadgeTeal")
        super.init(requestType: requestType)
        self.texture = textureTeal
        
        // Add animation
        
        let randomizer = arc4random_uniform(3)
